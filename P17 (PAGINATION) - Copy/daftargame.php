...
$game = query("SELECT*FROM game ORDER BY id DESC LIMIT $awaldata, $jumlahdataperhalaman");

$gam = query("SELECT*FROM game ORDER BY id DESC LIMIT 1");

// jika tombol search di klik
if (isset($_POST["search1"])) {
	$game = search1($_POST["keyword"]);
}
...
<body>
	<nav class="navbar navbar-expand-lg navbar-dark bg-dark" style="padding: 0px;">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
            ...
            <?php foreach ($gam as $row1) : ?>
              <marquee onmouseover="this.stop()" onmouseout="this.start()" ><a href="downloadgame.php?id=<?= $row1["id"]; ?>" target="blank"><?= $row1["Nama"] ?></a></marquee>
            <?php endforeach;?>
      </div>
	...
	<h1 style="text-align: center; color: green;">Daftar game</h1>
	...
	<div class="table-responsive">
		<table class="table table-bordered table-striped table-hover" border="1" cellpadding="10" cellspacing="0">
			...
			<tr>
				<td><?= $i ?></td>
				<td>
					<span class="badge badge-info"><a href="updategame.php?id=<?= $row["id"]; ?>">Update</a></span>
					<span class="badge badge-danger"><a href="deletegame.php?id=<?= $row["id"]; ?>" onclick="return confirm('Anda yakin akan menghapus datanya??');">Delete</a></span>
					
				</td>

				<td style="text-align: center;"><img src="img/<?= $row["Gambar"]; ?>" width="50" height="50"></td>
				<td><a href="downloadgame.php?id=<?= $row["id"]; ?>"><?= $row["Nama"] ?></a></td>
			</tr>
			<?php $i++; ?>
			<?php endforeach ; ?>
...
<br><br>
	<!-- navigasi -->
	<a class="btn btn-success btn-sm , tambah" href="tambahgame.php">Tambah data game</a>
